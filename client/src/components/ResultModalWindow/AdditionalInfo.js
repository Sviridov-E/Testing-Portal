...
export const AdditionalInfo = props => {
    ...
    const initialState = shouldBeDynamic ? 
        {
            status: true,
    ...
    const dynamicInfo = initialState.status ? (
        <div className="dynamic-info" ref={dynamicRef}>
            {
                initialState.data.map(({element}, ind) => <div key={ind}>{element}</div>)
            }
        </div>
    ) : null;
    ...
    useEffect(() => {
        ...
        const dynamicElements = Array.from(dynamicRef.current.children).map(item => item.children[0]);
        let instances = [];
    ...
    const staticInfo = typeof props.staticInfo === 'string' && props.staticInfo ? parseHtmlStr(props.staticInfo) : props.staticInfo;
